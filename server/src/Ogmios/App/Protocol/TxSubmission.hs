    ...
    localStateQueryClient await reply =
      ...
      where
        ...
        clientStIdle = pure $ do
            ...
            LSQ.SendMsgAcquire VolatileTip
                    ...
                    $ LSQ.ClientStQuerying
                        { LSQ.recvMsgResult = \case
                            Right networkUtxo -> do
                                reply
                                    $! newEvaluateTransactionResponse @era
                                        callback
                                        EvaluationFailure
                                        networkUtxo
                                        args
...
type HoistQuery proto era =
    forall r a qf crypto.
    (CardanoQueryResult crypto r ~ a) =>
    BlockQuery (ShelleyBlock (proto crypto) era) qf r ->
    BlockQuery (CardanoBlock crypto) qf a

{- | Run local-state queries in either Babbage, Conway or Dijkstra depending on where the network is at.
...
newEvaluateTransactionResponse callback onError (UTxO networkUtxo) args =
    case translateToNetworkEra @era args of
        Nothing ->
            error
                "impossible: arguments are not translatable to network era. \
            ...
            conwayToDijkstra =
                case (testEquality eraNetwork eraDijkstra, testEquality eraArgs eraConway) of
                    ...
                    _ ->
                        Nothing
         in
            sameEra <|> babbageToConway <|> conwayToDijkstra
