...
encodeGovAction = \case
    Cn.ParameterChange ancestor _pparamsUpdate guardrails ->
        encodeObject
            ( "type" .=
           ...
           <> "parameters" .=
                -- TODO(dijkstra): a Dijkstra-side encodePParamsUpdate is still
                -- needed (DijkstraPParams not ConwayPParams); mirrors the
                -- existing Conway.encodePParamsHKD stub.
                error "TODO(dijkstra): encodePParamsUpdate for Dijkstra"
           <> "guardrails" .=
