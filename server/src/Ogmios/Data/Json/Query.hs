...
data SomeQuery (f :: Type -> Type) block where
    ...
    SomeEffectfullQuery ::
        ...
        SomeQuery f block
    SomeCompoundQuery ::
        forall f block a b result crypto.
        ...
        SomeQuery f block
    SomeCompound2Query ::
        forall f block a b c result crypto.
        (crypto ~ BlockCrypto block) =>
        -- | First query to run.
        LSQ.Query block (QueryResult crypto a) ->
        -- | Second query to run, using result from the first one.
        ...
        ( \k summary ->
            encodeObject
                ( Shelley.encodeCredential "credential" k
                    <> "stakePool"
                    .=? OmitWhenNothing
                        (encodeSingleton "id" . Shelley.encodePoolId)
                        (poolDelegate summary)
                    <> "delegateRepresentative"
                    .=? OmitWhenNothing
                        (encodeObject . Conway.encodeDRep)
                        (drepDelegate summary)
                    <> "rewards"
...
encodeGovActionState st =
    "proposal"
        .= Conway.encodeGovActionId (gasId st)
        <> Conway.encodeProposalProcedure (gasProposalProcedure st)
        <> "since"
        .= (encodeSingleton "epoch" . encodeEpochNo) (gasProposedIn st)
        <> "until"
        .= (encodeSingleton "epoch" . encodeEpochNo) (gasExpiresAfter st)
        <> "votes"
        .= encodeMapAsList
            ( \issuer vote ->
                encodeObject
                    ( "issuer"
                        .= Conway.encodeVoter issuer
                        <> "vote"
                        .= Conway.encodeVote vote
                    )
            )
            ( mconcat
  ...
  where
    ...
    encodeIndividualPoolStake x =
        "stake"
            ...
            <> "totalStake"
            .= encodeCoin (fromCompact (Ledger.individualTotalPoolStake x))
            <> "vrf"
...
encodeRewardsProvenance rp =
    encodeObject
        ( "totalStake"
            ...
            <> "treasuryTax"
            .= encodeCoin (Coin $ deltaT1 rp)
            <> "totalRewards"
            .= encodeCoin (Coin $ rPot rp)
            <> "efficiency"
            .= encodeRational (eta rp)
            <> "stakePools"
            .= encodeMap Shelley.stringifyPoolId encodePoolRewardInfo (pools rp)
        )
  where
    encodePoolRewardInfo i =
        encodeObject
            $ "relativeStake"
            .= encodeRational (unStakeShare $ poolRelativeStake i)
            <> "blocksMade"
            .= encodeNatural (poolBlocks i)
            <> "totalRewards"
            .= encodeCoin (poolPot i)
            <> "leaderReward"
            .= encodeCoin (poolLeaderReward i)
            <> "delegators"
            .= encodeMapAsList
                ( \k v ->
                    encodeObject
...
deserialisePartialNewEpochState ::
    ...
    , Ledger.EraTxOut era
    , Ledger.EraGov era
    , Binary.DecShareCBOR (Ledger.CertState era)
    , Ledger.EraStake era
    ) =>
    ...
    encodeObject
        . encodeMapSeries
            ...
            ( \k (params, stake) ->
                encodeObject
                    ( "id"
                        .= Shelley.encodePoolId k
                        <> maybe mempty Shelley.encodePoolParams params
                        <> "stake"
                        .=? OmitWhenNothing encodeCoin stake
                    )
            )
...
{-# INLINEABLE eraMismatchOrResult #-}
...
parseQueryLedgerDelegateRepresentatives genResult =
  ...
  where
    ...
    mergeAll delegs =
        Map.merge
            ...
            ( Map.mapMaybeMissing $ \key coin ->
                let delegators = fromMaybe Set.empty (Map.lookup key delegs)
                 in case key of
    ...
    encodeDRepSummary ::
        ...
        DRepSummary ->
        Json
    encodeDRepSummary drep =
        encodeObject . (Conway.encodeDRep drep <>) . \case
    ...
    Json.withObject "eraStart" $ \obj -> do
        guard (null obj)
            ...
            . ( \case
                    ...
                    SomeShelleyEra ShelleyBasedEraDijkstra ->
                        LSQ.BlockQuery $ QueryAnytimeDijkstra GetEraStart
              )
...
parseQueryLedgerTip genResultInEraTPraos genResultInEraPraos =
    let query = "tip"
     in Json.withObject (toString @Text query) $ \obj -> do
            guard (null obj) $> \case
                SomeShelleyEra ShelleyBasedEraShelley ->
                    Just
                        $ SomeStandardQuery
...
parseQueryLedgerRewardAccountSummaries genResult =
    let query = "rewardAccountSummaries"
     in Json.withObject (toString @Text query) $ \obj -> do
            byScript <-
                obj
                    .:? "scripts"
                ...
                SomeShelleyEra ShelleyBasedEraConway ->
                    Just
                        $ SomeStandardQuery
                            ...
                            (genResultInEra (Proxy @ConwayEra))
                -- TODO(dijkstra): the GovState accessors in Cardano.Ledger.Conway.Governance
                -- are typed against ConwayEra; Dijkstra's GovState is parameterised over
                -- DijkstraEra. Needs an era-poly variant before this routes.
                SomeShelleyEra ShelleyBasedEraDijkstra ->
                    Nothing
                ...
                SomeShelleyEra ShelleyBasedEraConway ->
                    Just
                        $ SomeStandardQuery
                            ...
                            (genResultInEra (Proxy @ConwayEra))
                -- TODO(dijkstra): same as the unfiltered variant above; needs an
                -- era-poly GovState accessor surface.
                SomeShelleyEra ShelleyBasedEraDijkstra ->
                    Nothing
...
parseQueryLedgerLiveStakeDistribution genResult =
    let query = "liveStakeDistribution"
     in Json.withObject (toString @Text query) $ \obj -> do
            guard (null obj)
                $> ( \queryDef ->
                        Just
                ...
                . ( \case
                        SomeShelleyEra ShelleyBasedEraShelley ->
                            LSQ.BlockQuery $ QueryIfCurrentShelley GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraAllegra ->
                            LSQ.BlockQuery $ QueryIfCurrentAllegra GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraMary ->
                            LSQ.BlockQuery $ QueryIfCurrentMary GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraAlonzo ->
                            LSQ.BlockQuery $ QueryIfCurrentAlonzo GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraBabbage ->
                            LSQ.BlockQuery $ QueryIfCurrentBabbage GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraConway ->
                            LSQ.BlockQuery $ QueryIfCurrentConway GetStakeDistribution2
                        SomeShelleyEra ShelleyBasedEraDijkstra ->
                            LSQ.BlockQuery $ QueryIfCurrentDijkstra GetStakeDistribution2
                  )
...
parseQueryNetworkGenesisConfiguration (genByron, genShelley, genAlonzo, genConway) =
    Json.withObject "genesisConfiguration" $ \obj -> do
        ...
        case T.toLower era of
            ...
            (_unknownEra :: Text) -> do
                fail
                    "Invalid era parameter. Only 'byron', 'shelley', \
...
decodeStakingCredential decodeAsKeyOrScript =
    Json.withText "StakingCredential" $ \(encodeUtf8 -> str) ->
        asum
            [ decodeBase16 str
                >>= decodeAsKeyOrScript
            , decodeBech32 str
                >>= whenHrp
                    (`elem` [[humanReadablePart|stake|], [humanReadablePart|stake_test|]])
            ...
            <|> fail
                "Unable to decode credential. It must be either a base16-encoded \
...
decodeDRepCredential decodeAsKeyOrScript =
    Json.withText "DRepCredential" $ \(encodeUtf8 -> str) ->
        asum
            [ decodeBase16 str
                >>= decodeAsKeyOrScript
            , decodeBech32 str
                >>= whenHrp
                    (== [humanReadablePart|drep_vkh|])
                    decodeAsKeyHash
            , decodeBech32 str
                >>= whenHrp
                    (== [humanReadablePart|drep_script|])
                    decodeAsScriptHash
            ]
            <|> fail
                "Unable to decode credential. It must be either a base16-encoded \
    ...
    decodeFromWrappedJson o = do
        o .:? "language" >>= \case
            Just "native" -> do
                ...
                case (cbor, json) of
                    (Just bytes, _) ->
                        case decodeCborAnn @era "Script<Native>" decCBOR (toLazy bytes) of
                            Right script ->
...
decodeNativeScript ::
    ...
    Json.Value ->
    Json.Parser (Ledger.NativeScript era)
decodeNativeScript = Json.withObject "Script<Native>" $ \o -> do
    clause <- o .: "clause"
    case clause :: Text of
        "signature" ->
  ...
  where
    decodeRequireSignature o = do
        ...
        pure $ mkRequireSignature @era (Ledger.KeyHash from)
    decodeAllOf o = do
        xs <- StrictSeq.fromList <$> (o .: "from")
        mkRequireAllOf @era <$> traverse decodeNativeScript xs
    decodeAnyOf o = do
        xs <- StrictSeq.fromList <$> (o .: "from")
        mkRequireAnyOf @era <$> traverse decodeNativeScript xs
    decodeSomeOf o = do
        n <- o .: "atLeast"
        xs <- StrictSeq.fromList <$> (o .: "from")
        mkRequireMOf @era n <$> traverse decodeNativeScript xs
    decodeTimeExpire o =
        mkTimeExpire @era . SlotNo <$> (o .: "slot")
    decodeTimeStart o =
        mkTimeStart @era . SlotNo <$> (o .: "slot")
